<%@ page import="guestbook.Feedback" %>
...
<div class="fieldcontain ${hasErrors(bean: feedbackInstance, field: 'user', 'error')} required">
	...
	<g:select id="user" name="user.id" from="${guestbook.User.list()}" optionKey="id" required="" value="${feedbackInstance?.user?.id}" class="many-to-one"/>
</div>
